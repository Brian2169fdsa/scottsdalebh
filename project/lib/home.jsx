/* global React, PageShell, Slot, Ico, SectionHeading, StatBlock, Button, NotchBand,
   ValueItem, SplitFeature, CtaBand, Testimonials, InsuranceWall, ProgramCard, MARK, TEL, PHONE */
...
function HomeHero() {
  return (
    <section style={{ position: 'relative', overflow: 'hidden', contain: 'strict', height: 660, display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: 'var(--navy-900)', backgroundImage: 'url("assets/hero-bg.jpg")', backgroundSize: 'cover', backgroundPosition: 'center 30%', backgroundRepeat: 'no-repeat' }}>
      ...
      <div aria-hidden="true" style={{ position: 'absolute', right: -80, top: '50%', transform: 'translateY(-50%)', width: 320, height: 320, opacity: 0.06, pointerEvents: 'none' }}>
        <div style={{ position: 'absolute', left: '50%', top: 0, transform: 'translateX(-50%)', width: 100, height: 320, background: '#fff', borderRadius: 16 }} />
        <div style={{ position: 'absolute', top: '50%', left: 0, transform: 'translateY(-50%)', width: 320, height: 100, background: '#fff', borderRadius: 16 }} />
      </div>
...
function ProgramsHome() {
  const progs = [
    { badge: 'PHP', title: 'Partial Hospitalization', icon: 'sun', body: 'Structured daytime treatment with the comfort of returning home each evening.', href: 'programs.html#php' },
    { badge: 'IOP', title: 'Intensive Outpatient', icon: 'calendar-clock', body: 'Flexible sessions that fit around work, school, and family life.', href: 'programs.html#iop' },
    { badge: 'OP', title: 'Outpatient', icon: 'users', body: 'Ongoing therapy and support as you build lasting momentum in recovery.', href: 'programs.html#outpatient' },
    { badge: 'E-IOP', title: 'Evening IOP', icon: 'moon', body: 'Evidence-based care that keeps your life moving — evenings, after work.', href: 'programs.html#evening' },
  ];
  return (
    <section className="sbhg-section" style={{ background: 'var(--cream-50)' }}>
      <div className="sbhg-container">
        ...
        <p style={{ textAlign: 'center', marginTop: 24, fontFamily: 'var(--font-body)', fontSize: 13, color: 'var(--gray-600)' }}>[NEEDS-CLIENT] Confirm SBHG's actual levels of care before launch.</p>
      </div>
    </section>
  );
}
...
function FullBleedImage() {
  return (
    <section style={{ position: 'relative', width: '100%', height: 900, overflow: 'hidden', contain: 'strict', backgroundColor: 'var(--navy-900)', backgroundImage: 'url("assets/mh-entrance.jpg")', backgroundSize: 'cover', backgroundPosition: 'center 40%', backgroundRepeat: 'no-repeat' }} aria-label="Recovery statistics over the doors of Scottsdale Behavioral Health Group">
      ...
      <div className="sbhg-container" style={{ position: 'relative', height: '100%', display: 'grid', gridTemplateColumns: '1.4fr 0.6fr', gap: 72, alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: 26 }}>
          ...
          <Button variant="primary" size="lg" href="mental-health.html" style={{ minWidth: 220, letterSpacing: '.06em' }}>Learn More</Button>
        </div>
      </div>
      <div style={{ position: 'absolute', bottom: -1, left: 0, right: 0 }}><NotchBand height={42} /></div>
    </section>
  ...
  return (
    <PageShell active="Home">
      ...
      <SplitFeature
        ...
        cta="Explore Mental Health" ctaHref="mental-health.html" slotId="home-mh" tone="calm" bg="var(--cream-50)"
      />
      <CtaBand />
      <SplitFeature
        ...
        cta="Explore Substance Abuse" ctaHref="substance-abuse.html" slotId="home-sa" tone="canyon" flip bg="var(--white)"
      />
      <ValuesRow />
      <Testimonials />
      <ProgramsHome />
