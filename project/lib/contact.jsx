/* global React, PageShell, SectionHeading, Slot, Ico, Button, Input, Select, IconCircle, DS, useState */

function ContactSection() {
  const [sent, setSent] = useState(false);
  const { Input: In, Select: Sel, Button: Btn } = window.DesignSystem_8429f7;
  const rows = [
    { icon: 'map-pin', label: 'Visit', value: '[NEEDS-CLIENT] Scottsdale, Arizona' },
    { icon: 'phone', label: 'Call', value: '(480) 555-0100' },
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
            {rows.map((r) => (
              <div key={r.label} style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
                <span style={{ width: 52, height: 52, borderRadius: '50%', border: '1.5px solid var(--gold-500)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', color: 'var(--gold-700)', flexShrink: 0 }}><Ico name={r.icon} size={22} c="var(--gold-700)" /></span>
                <div>
        ...
        <div style={{ background: 'var(--cream-50)', border: '1px solid var(--gray-200)', borderRadius: 'var(--radius-card)', padding: 36, boxShadow: 'var(--shadow-soft)' }}>
          {sent ? (
            ...
            <form onSubmit={(e) => { e.preventDefault(); setSent(true); }} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18 }}>
              <In label="First Name" placeholder="Jane" required />
              <In label="Last Name" placeholder="Doe" required />
              <In label="Phone" type="tel" placeholder="(480) 555-0102" required />
              <In label="Email" type="email" placeholder="jane@email.com" required />
              <div style={{ gridColumn: '1 / -1' }}><Sel label="How can we help?" placeholder="Select a topic" options={['Verify insurance', 'Ask about programs', 'Speak with admissions', 'Something else']} /></div>
              <div style={{ gridColumn: '1 / -1' }}><In label="Message" placeholder="Tell us a little about what you're looking for" /></div>
              <label style={{ gridColumn: '1 / -1', display: 'flex', gap: 10, alignItems: 'flex-start', fontFamily: 'var(--font-body)', fontSize: 13, color: 'var(--gray-600)', lineHeight: 1.5 }}>
                <input type="checkbox" style={{ marginTop: 3, accentColor: 'var(--gold-700)' }} /> I consent to be contacted by phone, email, or SMS about treatment.
              </label>
              <div style={{ gridColumn: '1 / -1', marginTop: 2 }}><Btn variant="primary" size="lg" type="submit" style={{ width: '100%' }}>Send Message</Btn></div>
    ...
    <section style={{ background: 'var(--navy-900)', padding: '80px 0' }}>
      <div className="sbhg-container" style={{ textAlign: 'center' }}>
        ...
        <div style={{ marginTop: 40 }}>
          <Button variant="primary" size="lg" href="tel:+14805550100">Call (480) 555-0100</Button>
        </div>
      </div>
    </section>
  );
  ...
  return (
    <section className="sbhg-section" style={{ background: 'var(--cream-50)' }}>
      <div className="sbhg-container">
        ...
        <p style={{ textAlign: 'center', marginTop: 24, fontFamily: 'var(--font-body)', fontSize: 13, color: 'var(--gray-600)' }}>[NEEDS-CLIENT] Replace with SBHG facility photography.</p>
      </div>
    </section>
  );
}
...
function ContactPage() {
  return (
    <PageShell active="Contact" showVerify={false} hero={{
