...
import { SectionHeading } from '../ds/index';
import { Ico } from '../components/Ico';

const MH_DISORDERS = [
  ['PTSD & Trauma', 'Process what happened in a safe, trauma-informed setting.', '#disorders'],
  ['Anxiety', 'Quiet the noise and rebuild a sense of steady ground.', '#disorders'],
  ['Depression', 'Find your way back toward energy, meaning, and connection.', '#disorders'],
  ['Bipolar Disorder', 'Stabilize mood with integrated psychiatry and therapy.', '#disorders'],
  ['Personality Disorders', 'Long-term skills for relationships and emotional regulation.', '#disorders'],
];
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 22, marginTop: 48 }}>
          {cards.map(([icon, title, body]) => (
            <div key={title} style={{ display: 'flex', flexDirection: 'column', gap: 14, padding: 28, background: 'var(--cream-50)', border: '1px solid var(--gray-200)', borderRadius: 'var(--radius-card)', boxShadow: 'var(--shadow-soft)' }}>
              <span style={{ width: 52, height: 52, borderRadius: '50%', border: '1.5px solid var(--gold-500)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}><Ico name={icon} size={22} c="var(--gold-700)" /></span>
