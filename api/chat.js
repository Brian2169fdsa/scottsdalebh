...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) return res.status(500).json({ error: 'Invalid API key - check ANTHROPIC_API_KEY in Vercel' });
      if (response.status === 404) return res.status(500).json({ error: 'Model not found - check model ID' });
      if (response.status === 429) return res.status(500).json({ error: 'Rate limited - please try again' });
